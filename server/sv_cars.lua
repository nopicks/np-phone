function getCars(pCharacterId)
    ...
    -- local query = [[
    --     SELECT model, `[-277.5246887207,-889.99249267578,30.827133178711]` as 'location', `state` as 'parking_state', `garage` as 'parking_garage', `plate`, `damage`, `type` as type
    --     FROM _vehicle WHERE cid = ?
    -- ]]
    -- local pResult = Await(SQL.execute(query, pCharacterId))

    -- for _, v in pairs(pResult) do
    --     local conf = exports["np-showrooms"]:getSingleCarConfig(v.model)
    --     if conf then
    --         v.name = conf.name
    --         v.brand = conf.brand
    --     else
...
-- exports.ghmattimysql:execute("SELECT * FROM characters_cars WHERE cid = @cid", {['username'] = player, ["cid"] = character.id}, function(result)
--     if (result) then
...
--                 amountduenow = math.ceil(v.purchase_price / 10 * (last_payment / 7))
--             end
--             t = { ["id"] = v.id, ["model"] = v.model, ["name"] = v.name, ["license_plate"] = v.license_plate, ["vehicle_state"] = v.vehicle_state, ["engine_damage"] = v.engine_damage, ["body"] = v.body_damage, ["current_garage"] = curGarage, ["data"] = json.decode(v.data), ["fuel"] = v.fuel, ["payment"] = v.financed, ["last_payment"] = last_payment, ["amount_due"] = amountduenow, ["coords"] = json.decode(v.coords), }
--             vehicles[v.id] = t
--         end
--     end
